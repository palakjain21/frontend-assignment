...
console.log(canvas.components, "board");
  ...
  useEffect(() => {
 canvas.observe(setComponentPositions);
    
  });

  const renderSquare = (i, j, count) => {

    const x = i;
    const y = j;
    const componentsAtPosition = canvas.components?.filter(
      (component) => component.position.x === x && component.position.y === y
    );
    // console.log(componentsAtPosition, "render");

    return (
      <div key={`${i}-${j}`} style={{ border: '1px solid black' }}>
        <BoardSquare x={x} y={y} canvas={canvas}>
          {componentsAtPosition?.map((component) => (
            <Piece compId={component.id} type={component.type} />
          ))}
        </BoardSquare>
      </div>
    );
