...
console.log(canvas.components, "board");
  const [componentPositions, setComponentPositions] = useState(
    canvas.components
  );
  const [[buttonX, buttonY], setButtonPos] = useState(canvas.ComponentPosition)   //taking the intial position of the components from the canvas instance
  ...
  function renderSquare(i,j,count) {
    const x = i
    const y = j
    const componentToRender=canvas.components[JSON.stringify([x,y])]
    return (
      <div key={`${i}-${j}` } styles={{border:"1px solid black"}}>
        <BoardSquare x={x} y={y} canvas={canvas}>
          {/* <Piece isButton={x === buttonX && y === buttonY} /> */}
          <Piece component={componentToRender} canvas={canvas} coordinates={[x,y]}/>
        </BoardSquare>
      </div>
    );
  };
